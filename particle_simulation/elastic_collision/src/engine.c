...
{
    float dist_squared = (b1->x - b2->x) * (b1->x - b2->x) + (b1->y - b2->y) * (b1->y - b2->y);
...
void do_elastic_collision(ball *b1, ball *b2)
{
    ...
    if (magnitude_vel_b1 < 1e-5 && magnitude_vel_b2 < 1e-5)
        return;

    // Check for interpenetration of spheres 
    if (check_ball_collision(b1, b2))
    {
        float overlap = (b1->radius + b2->radius) - sqrt((b1->x - b2->x) * (b1->x - b2->x) + (b1->y - b2->y) * (b1->y - b2->y));
        float total_mass = b1->mass + b2->mass;
        float correction_b1 = overlap * (b2->mass / total_mass);
        float correction_b2 = overlap * (b1->mass / total_mass);
        // Move balls apart along the line connecting their centers
        float dx = b2->x - b1->x;
        float dy = b2->y - b1->y;
        ...
        b1->x -= nx * correction_b1;
        b1->y -= ny * correction_b1;
        b2->x += nx * correction_b2;
        b2->y += ny * correction_b2;
    }
...
void main_loop(SDL_Renderer *renderer, int NB_BALLS)
{
    ...
    while (running)
    {
        ...
        for (int i = 0; i < NB_BALLS; i++)
        {
            ...
            {
                if (check_ball_collision(tab_balls[i], tab_balls[j]))
                {
